...
BEGIN
  ...
  -- Assign role: 'donor' for helpers, 'recipient' for users
...
CREATE POLICY "Users and donors view requests by proximity"
ON public.aid_requests
FOR SELECT
USING (
  status = 'open' AND (
    -- Recipients can see all
    public.has_role(auth.uid(), 'recipient') OR
    -- Donors see within 30km
    (
